...
- 支持云端 `life` 发起任务、本机 Codex worker 消费任务并回传结果。
- 支持已完成或失败任务删除：云端先标记 `delete_requested`，本机 worker 删除 `generated/codex-media/<requestId>` 后清理队列记录。
- worker 内置删除巡检，即使正在生成图片/视频，也会定期处理删除请求。
- 支持 Windows 计划任务开机自启。
...
- 对明显的“生成图片/视频”请求会直接创建任务；对长段视觉提示词会先询问确认。
